...
func buildSummaryPods(cinfos map[string]cadvisorapiv2.ContainerInfo) []PodStats {
	...
	for _, cinfo := range cinfos {
		...
		if containerName == leaky.PodInfraContainerName {
			...
			stats.UserDefinedMetrics = append(stats.UserDefinedMetrics, convertInfoV2ToUserDefinedMetrics(&cinfo)...)
		}
	}
...
func convertInfoV2ToUserDefinedMetrics(info *cadvisorapiv2.ContainerInfo) []UserDefinedMetric {
	type specVal struct {
		ref     UserDefinedMetricRef
		...
		for name, values := range stat.CustomMetrics {
			...
			for _, value := range values {
				...
				if specVal.valType == cadvisorapiv1.IntType {
					var floatVal float64 = float64(value.IntValue)
					specVal.value = floatVal
				}
			}
		}
	}
	...
	for _, specVal := range udmMap {
		udm = append(udm, UserDefinedMetric{
			UserDefinedMetricRef: specVal.ref,
			Value:                specVal.value,
		})
	}
	return udm
